/// Defines Managed Objects to be used in the
/// Virtual Machine and Garbage Collector.
///
/// We need this for multiple reasons:
///     1. Dealing with the borrow checker is awful for the VM,
///        so instead we handle raw pointers.
///     2. We need to define different structures to handle the
///        garbage collection.
///
...
impl<T: 'static + Trace + PartialEq + ?Sized> PartialEq<Gc<T>> for Gc<T> {
