...
namespace mango {
        ...
        while (true) {
            ...
            for (const auto &i : temporary) {
                ...
                if (find(nonterminals.begin(), nonterminals.end(), X) != nonterminals.end()) {
                    for (const auto &production : grammar[X]) {
                        token seek_token = (b != epsilon) ? b : z;
    ...
    void generator::gitems() {
        CustomItemSet init_item(item(Mango, {StatementSuite}, 0, eof));
    ...
    void generator::gtable_files() {
        ...
        for (const auto &p : taction) {
            ss << "\t\t{ {" << p.first.first << ", " << token_map[p.first.second] << "}, " << p.second
               << "},\n";
        }
        ss << "};\n";
        ...
        for (const auto &p : grammar) {
            ...
            for (const auto &production : productions) {
                ...
                reverse(production_reversed.begin(), production_reversed.end());
                int offset = 1;
                ...
                ss << "\t\t\t\tauto* node = new " << token_map[key] << i << " {";
                reverse(tempvars.begin(), tempvars.end());
                node_pointer_map[token_map[key] + to_string(i)] = tempvars;
